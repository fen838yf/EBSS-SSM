...
from ultralytics.nn.modules.DCNv4_op.DCNv4 import DCNv4
from ultralytics.nn.modules.PlainUSR import RepMBConv, LocalAttention
from ultralytics.nn.modules.SPJFB import SPJFrequencyBlock
from ultralytics.nn.modules.UniConvNet import UniConvBlock
from ultralytics.nn.modules.filc import FAT_Block
from ultralytics.nn.modules.gcconv import GCConv
...
class C2PSAMamba(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, e: float = 0.5, global_ratio=0.8,
                 local_ratio=0.2):
        """Initialize C2PSA module.
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c1, 1)
        self.cv3 = MambaModule(dim=c1, global_ratio=global_ratio, local_ratio=local_ratio)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        b = self.m(b)
        # return self.cv2(torch.cat((a, b), 1))
        k = self.cv3(self.cv2(torch.cat((a, b), 1)))
        return self.triplet_attention(k)

class SPPFMamba(nn.Module):
    """Spatial Pyramid Pooling - Fast (SPPF) layer for YOLOv5 by Glenn Jocher."""

    def __init__(self, c1: int, c2: int, k: int = 5, n: int = 3, shortcut: bool = False, global_ratio=0.8,
                 local_ratio=0.2):
        """Initialize the SPPF layer with given input/output channels and kernel size.
        ...
        self.cv1 = Conv(c1, c_, 1, 1, act=False)
        self.cv2 = Conv(c_ * (n + 1), c2, 1, 1)
        self.cv3 = MambaModule(dim=c2, global_ratio=global_ratio, local_ratio=local_ratio)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.n = n
        self.add = shortcut and c1 == c2
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = [self.cv1(x)]
        y.extend(self.m(y[-1]) for _ in range(getattr(self, "n", 3)))
        y = self.cv3(self.cv2(torch.cat(y, 1)))
        return y + x if getattr(self, "add", False) else y
